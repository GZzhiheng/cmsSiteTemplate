    ...
    RemoteImage.prototype = {
        ...
        setImage: function (img) {
            ...
            if (src) {
                ...
                $G("marginTop").value = parseInt(img.style.marginTop) || '0';
                $G("marginRight").value = parseInt(img.style.marginRight) || '0';
                $G("marginBottom").value = parseInt(img.style.marginBottom) || '0';
                $G("marginLeft").value = parseInt(img.style.marginLeft) || '0';
        ...
        setPreview: function () {
            ...
            var getSize = function (input, fullProp) {
                if (/^\d+%$/.test(input)) {
                    //百分比
                    return parseInt(input) * preview[fullProp] / 100;
                } else if (/^\d+(px)?$/.test(input)) {
            ...
            ow = getSize(ow, 'offsetWidth');
            oh = getSize(oh, 'offsetHeight');
        ...
        getInsertList: function () {
            var getSize = function (prop, input) {
                if (!input) return '';
                if (/^\d+$/) {
                    return prop + ':' + input + 'px;';
                }
                return prop + ':' + input + ';';
            }
            ...
            if (data['url']) {
                return [{
                    ...
                    title: data['title'] || '',
                    alt: data['title'] || '',
                    style: getSize('width', data['width']) +
                    getSize('height', data['height']) +
                    "margin-top:" + (data['marginTop'] || 0) + "px;" +
                    "margin-right:" + (data['marginRight'] || 0) + "px;" +
                    "margin-bottom:" + (data['marginBottom'] || 0) + "px;" +
                    "margin-left:" + (data['marginLeft'] || 0) + "px;"
                }];
            } else {
                return [];
            }
        ...
        initUploader: function () {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (typeof editor.getOpt('imageUploadCallback') == 'function') {
                        editor.getOpt('imageUploadCallback').call(editor, json, function (link) {
                            _this.imageList.push({url: link});
